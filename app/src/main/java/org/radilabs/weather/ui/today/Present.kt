...
private fun ForecastPoint.toHour(offsetSeconds: Int): HourPoint {
    ...
    return HourPoint(
        ...
        precipChance = precipitation.probabilityPercent?.roundToInt() ?: 0,
    )
}
...
private fun formatWind(wind: Wind): String {
    ...
    return dir + "${wind.speedMps.roundToInt()} m/s"
}
...
private fun formatPrecip(rainMm: Double?, snowMm: Double?): String {
    val rain = rainMm
    val snow = snowMm
    return when {
        rain != null && snow != null -> "${formatMm(rain)} rain / ${formatMm(snow)} snow"
        rain != null -> formatMm(rain)
        snow != null -> "${formatMm(snow)} snow"
        else -> "—"
    }
}
...
private fun formatVisibility(metres: Double?): String {
